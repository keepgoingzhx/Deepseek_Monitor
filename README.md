...
推荐使用登录态同步，不需要填写 DeepSeek API Key：

1. 启动桌面端：`npm start`
2. 点击主界面的“登录”按钮。
3. 在弹出的 DeepSeek 官方页面里自行登录账号。
4. 登录完成后保持页面片刻，DeepSeek Monitor 会自动识别登录态并同步用量。
5. 首次同步成功后，登录窗口会自动关闭；之后应用会在后台定时更新。

如果登录后没有马上看到数据，可以点击“同步”手动触发一次。打开登录页期间，应用也会每 15 秒检测一次登录态并尝试同步。

API Key 是可选功能，主要用于本地代理转发请求和余额查询。
...
不要把 DeepSeek 账号密码发给任何人。桌面端主界面的“登录”按钮会打开 DeepSeek 官方 Usage 页面，你在本机窗口里自行登录。
